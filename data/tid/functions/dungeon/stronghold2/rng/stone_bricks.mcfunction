...
#If there is air above, consider generating a grass patch or gravel patch
execute if predicate tid:random_01 if block ~ ~1 ~ air run function tid:dungeon/stronghold2/rng/grass_patch
execute if predicate tid:random_01 if block ~ ~1 ~ air positioned ~ ~1 ~ run function tid:dungeon/stronghold2/rng/gravel_patch
