...
execute if score @s x matches -30..30 if score @s z matches -30..30 run scoreboard players set close_to_spawn value 1

execute if predicate tid:random_07 unless score close_to_spawn value matches 1 run setblock ~1 ~ ~1 minecraft:structure_block{name: "tid:jungle/totem_9", posX: -1, posY: 0, posZ: -1, ignoreEntities: 0b, powered: 0b, mode: "LOAD", showair: 0b, showboundingbox: 0b}

#Nearly guaranteed one Illusioner tower, placed a good distance from spawn
execute unless score close_to_spawn value matches 1 unless entity @e[tag=haunted,predicate=tid:in_layer_jungle] run setblock ~1 ~ ~1 minecraft:structure_block{name: "tid:jungle/totem_8", posX: -1, posY: 0, posZ: -1, ignoreEntities: 0b, powered: 0b, mode: "LOAD", showair: 0b, showboundingbox: 0b}
