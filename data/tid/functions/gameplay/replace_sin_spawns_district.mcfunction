...
execute if predicate tid:random_50 unless entity @e[type=piglin_brute,tag=captain,distance=0..30] positioned ~ 200 ~ if entity @e[type=area_effect_cloud,tag=sin_district,distance=0..15] run scoreboard players set temp value 16

execute if predicate tid:random_05 positioned ~ 200 ~ if entity @e[type=area_effect_cloud,tag=sin_district,distance=0..15] run scoreboard players set temp value 16
