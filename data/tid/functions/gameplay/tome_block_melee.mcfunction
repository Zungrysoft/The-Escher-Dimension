...
execute if entity @s[nbt={SelectedItem:{tag:{tome_block:1b}}}] if score @s tome_use matches 1.. unless score temp value matches 1.. run function tid:gameplay/fail_sound_2

#Check for using the book with sufficient ammo
execute if entity @s[nbt={SelectedItem:{tag:{tome_block:1b}}}] if score @s tome_use matches 1.. if score temp value matches 1.. run function tid:gameplay/tome_block_use
